...
S <- 20
mean_SPCE <- sd_SPCE <- lo <- hi <- bias <- numeric(S)
...
spce_drawU_bb <- function(m, X, t0,
                          eta0, eta_x, eta_a, eta_u, k_draws, gamma0, gamma1,
                          LU ) {
  ...
  Umat <- matrix(rbinom(N*LU, 1, rep(pU, each = LU)), nrow = N, ncol = LU)
...
#quantile
q <- quantile(psi_all, c(0.025, 0.975), names = FALSE)
lo[s] <- q[1]; hi[s] <- q[2]

mean_SPCE[s] <- mean(psi_all)
sd_SPCE[s]   <- sd(psi_all)
bias[s] <- mean_SPCE[s] - true_spce_aft 

}

#ci_SPCE <- quantile(psi_all, c(0.025, 0.975))

#mean_SPCE; sd_SPCE; ci_SPCE
mean_SPCE
mean(mean_SPCE)
mean(sd_SPCE)
bias;mean(bias)
